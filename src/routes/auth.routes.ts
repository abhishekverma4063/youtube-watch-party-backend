...
const JWT_SECRET: string = process.env.JWT_SECRET || 'supersecretwatchpartykey2026';
const REFRESH_SECRET: string = process.env.REFRESH_SECRET || 'supersecretrefreshkey2026';
...
router.get('/me', async (req, res) => {
  try {
    ...
    if (!tokenPart) return res.status(401).json({ error: 'Unauthorized' });
